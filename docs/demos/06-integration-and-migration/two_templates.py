...
@app.callback(
    args=dict(
        ...
        continent=gapminder_tpl.checklist_input(continents, value=continents, label="Continents"),
...
def tips_callback(smoker):
    ...
    return px.histogram(
        plot_tips_df, x="total_bill", y="tip", color="sex", marginal="rug", hover_data=tips_df.columns
    )


app.layout = dbc.Container(fluid=True, style={"padding": 20}, children=[
    html.Div(children=[
        html.H2("Data Explorer"),
        html.Hr(),
        dbc.Tabs([
            dbc.Tab(dbc.Card(gapminder_tpl.children, body=True), label="Gapminder"),
