...
Dash express makes it possible to include a component instance in place of a component id in the `Input`, `State`, and `Output` dependency objects.  This feature is currently provided by the special `dx.Input`, `dx.State`, `dx.Output` classes. Dash Express apps should use these classes in place of the Dash 1 `dash.dependency.Input`, `State`, and `Output` classes.  
...
from dash_express.component_plugins.base import ComponentPlugin
