...
from . import (
    ...
    mock_fn_with_return,
    make_letters,
    make_letters_grouping,
    make_numbers_grouping,
)
...
def check_component_props_as_groupings(
    ...
    if input_form == "dict":
        ...
        grouped_input_deps = {k: v for k, v in zip(only_input_keys, grouped_input_deps)}

        state_keys = make_letters(len(grouped_state_deps), upper=False)
        grouped_state_deps = {k: v for k, v in zip(state_keys, grouped_state_deps)}
        input_value_keys = only_input_keys + state_keys

        grouped_input_values = {
            k: v for k, v in zip(input_value_keys, grouped_input_values)
        }
    ...
    if output_form == "scalar":
        ...
        grouped_output_deps = {
            k: v
            for k, v in zip(
                make_letters(len(grouped_output_deps), upper=True), grouped_output_deps
            )
        }
